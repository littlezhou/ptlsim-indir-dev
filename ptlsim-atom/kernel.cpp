...
#define MAX_PROC_MAPS_SIZE 16*1024*1024
  ...
  while (p && (*p)) {
    ...
    bool stack = (((pattr && strequal(pattr, "[stack]")) || 
                   (line == (linecount-3))) ? MAP_STACK : 0);
     // was linecount-2 
    // DSM: only grab the stack once
  //  bool stack = ((pattr && strequal(pattr, "[stack]")) ? MAP_STACK : 0);
    //if(stack) { cout << "stack info: ", s, endl, flush; }
    map->prot = 
      ((rperm == 'r') ? PROT_READ : 0) |
...
void AddressSpace::resync_with_process_maps() {
  ...
  logfile << "stack_min_addr: 0x", hexstring(stack_min_addr,48), endl;
  logfile << "stack_max_addr: 0x", hexstring(stack_max_addr,48), endl;
  logfile << "init_stack_min_addr: 0x", hexstring(init_stack_min_addr,48), endl;
...
extern "C" void* ptlsim_preinit(void* origrsp, void* nextinit) {
 ...
 //stack_max_addr = ceil(ctx.commitarf[REG_rsp], 256*1024*1024);
  stack_max_addr = ceil(ctx.commitarf[REG_rsp],PAGE_SIZE);
