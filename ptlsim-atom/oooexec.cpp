...
void IssueQueue<size, operandcount>::clock() {
  ...
  stats.ooocore.issue.width.ready[allready.popcount()]++;
...
int ReorderBufferEntry::issue() {
  ...
  // Try msbindex to avoid chewing up Load ports prematurely 
  ...
  assert(rb.ready());
  if likely ((!st || (st && load_store_second_phase)) && (uop.rc != REG_imm)){  if(operands[RC]->rob && !operands[RC]->rob->broadcast  ){ /* logfile << "operand[RD]->broadcast false: ", isload(rc.rob->uop.opcode), endl; */ } assert(rc.ready()); }
  if likely (!st) {   if(operands[RS]->rob && !operands[RS]->rob->broadcast) { /* logfile << "operand[RS]->broadcast false: ", isload(ra.rob->uop.opcode), endl; */ }
assert(operands[RS]->ready()); }
  if likely (ra.nonnull()) {
    //if(operands[RA]->rob && !operands[RA]->rob->broadcast) logfile << "operand[RA]->broadcast false: ", isload(ra.rob->uop.opcode),  endl;
  ...
  if likely ((!uop.rbimm) & (rb.nonnull())) { 
    //  if(operands[RB]->rob && !operands[RB]->rob->broadcast ) logfile << "operand[RB]->broadcast false: ",  isload(rb.rob->uop.opcode) , endl;
  ...
  if unlikely ((!uop.rcimm) & (rc.nonnull())) {
    // if(operands[RC]->rob && !operands[RC]->rob->broadcast  ) logfile << "operand[RC]->broadcast false: ",  isload(rc.rob->uop.opcode) , endl;
  ...
  if unlikely ((ra.flags | rb.flags | rc.flags) & FLAG_INV) {
    ...
    if unlikely (ld|st) {
      ...
      if likely (ld) {
        ...
        completed = issuestore(*lsq, origvirt, radata, rbdata, rcdata, operands[2]->state != PHYSREG_WAITING, pteupdate);
      }
      ...
      if unlikely (mispredicted) {
       ...
       if(isclass(uop.opcode, OPCLASS_COND_BRANCH)) {
        if(uop.predinfo.ripafter == realrip)
        {
             ...
             ++currBranch->pred_not_taken_and_not_taken;
        }else
        {
...
int ReorderBufferEntry::issuestore(LoadStoreQueueEntry& state, Waddr& origaddr, W64 ra, W64 rb, W64 rc, bool rcready, PTEUpdate& pteupdate) {
  ...
  bool annul;
  //if(logenable) { logfile << "issuestore: rob ", idx,  "  rip: 0x", hexstring(uop.rip.rip, 24), " rcready: ", rcready, endl; }   
    ...
    if (iqslot != 0)
    {
      ...
      for(int i=0;i<ISSUE_QUEUE_SIZE;++i) 
      {
        ...
        if(isclass(rob.uop.opcode, OPCLASS_COND_BRANCH))
        {
           ...
           if(null == binfo)
           {
             BranchInfo* temp = new BranchInfo(rob.uop.rip.rip);
             branchHash.add(rob.uop.rip.rip, temp);
             currBranch = temp;
           }
          else { currBranch = *binfo; }
...
W64 ReorderBufferEntry::annul(bool keep_misspec_uop, bool return_first_annulled_rip) {
  ...
  for (;;) {
    ReorderBufferEntry& annulrob = ROB[idx];
    if(annulrob.issued) {  stats.ooocore.issue.wrong_path_instrs++; }
    lastrob = &annulrob;
