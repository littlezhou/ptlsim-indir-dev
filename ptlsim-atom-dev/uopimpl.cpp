...
#define make_exp_aluop_ex(name, expr) \
template <typename T, int genflags> \
struct name { \
  T operator ()(T ra, T rb, T rc, W16 raflags, W16 rbflags, W16 rcflags, byte& cf, byte& of) { \
    cf = 0; of = 0; W64 rd; if((raflags & FLAG_SF) && !(0x8000000000000000 & ra)) { ra = (W64s)((W32s)ra); } \
    if((rbflags & FLAG_SF) && !(0x8000000000000000 & rb)) { rb = (W64s)((W32s)rb); } \
    if((rcflags & FLAG_SF) && !(0x8000000000000000 & rc)) { rc = (W64s)((W32s)rc); } \
    expr; return rd; \
  } \
}
...
#define make_x86_aluop2_ex(name, opcode, pretext) \
template <typename T, int genflags> \
struct name { \
  T operator ()(T ra, T rb, T rc, W16 raflags, W16 rbflags, W16 rcflags, byte& cf, byte& of) { \
  if((raflags & FLAG_SF) && !(0x8000000000000000 & ra)) { ra = (W64s)((W32s)ra); } \
  if((rbflags & FLAG_SF) && !(0x8000000000000000 & rb)) { rb = (W64s)((W32s)rb); } \
  if((rcflags & FLAG_SF) && !(0x8000000000000000 & rc)) { rc = (W64s)((W32s)rc); } \
  if (genflags & (SETFLAG_CF|SETFLAG_OF)) \
      asm(pretext #opcode " %[rb],%[ra]; setc %[cf]; seto %[of]" : [ra] "+q" (ra), [cf] "=q" (cf), [of] "=q" (of) : [rb] "qm" (rb), [rcflags] "rm" (rcflags)); \
...
#define make_x86_aluop_all_sizes(name, opcode, setflags, pretext) \
  make_x86_aluop2_ex(x86_op_ ## name, opcode, pretext); \
...
#define make_exp_aluop3_all_sizes_all_shifts(ptlopcode, name, expr, setflags) \
  make_exp_aluop_ex(exp_op_ ## name, (expr)); \
