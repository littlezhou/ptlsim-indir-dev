...
static const int MACHINE_WIDTH = 4; // DEFAULT: 4
...
namespace OutOfOrderModel {
  ...
  static const int FU_COUNT = 10;
  static const int LOADLAT = 4;
 ...
 W32   FU_FPU3       = (1 << 9);
  ...
  static const int LOAD_FU_COUNT = 4;
...
#undef ANYINT
  ...
  const int MAX_ISSUE_WIDTH = MACHINE_WIDTH;
...
#define BIG_ROB
  ...
  const int FETCH_WIDTH = MACHINE_WIDTH;

  //
  // Frontend (Rename and Decode)
  //
  const int FRONTEND_WIDTH = MACHINE_WIDTH;
  const int FRONTEND_STAGES = 5;

  //
  // Dispatch
  //
  const int DISPATCH_WIDTH = MACHINE_WIDTH;

  //
  // Writeback
  //
  const int WRITEBACK_WIDTH = MACHINE_WIDTH;

  //
  // Commit
  //
  const int COMMIT_WIDTH = MACHINE_WIDTH;
...
#define SMT_ENABLE_LOAD_HOISTING
  ...
  enum {
    ...
    EVENT_FOUND_NONBLOCKING_ANCESTOR,
    EVENT_FOUND_NONBLOCKING_ANCESTOR_DESC,
    EVENT_FOUND_NONBLOCKING_SUCCESSOR,
    EVENT_FOUND_NONBLOCKING_RSP,
    EVENT_FOUND_NONBLOCKING_SUCCESSOR_UNMARK,
...
#define InitClusteredROBList(name, description, flags) \
  ...
  static const int ISSUE_QUEUE_SIZE = IQ_SIZE;
  ...
  static const int DISPATCH_DEADLOCK_COUNTDOWN_CYCLES = 512;
...
#define MAX_SMT_CORES 1
  ...
  const Cluster clusters[MAX_CLUSTERS] = {
    ...
    {"fp",    4, (FU_FPU0|FU_FPU1|FU_FPU2|FU_FPU3)},
  };
...
struct OutOfOrderCoreStats { // rootnode:
  ...
  struct dispatch {
    ...
    W64 nonready_sources[OutOfOrderModel::MAX_OPERANDS+1]; // histo: 0, OutOfOrderModel::MAX_OPERANDS, 1
    W64 width[OutOfOrderModel::DISPATCH_WIDTH+1]; // histo: 0, OutOfOrderModel::DISPATCH_WIDTH, 1
    W64 marked_block_count[256]; // histo: 0, 255, 1
    W64 marked_successors_count[256]; // histo: 0, 255, 1
    W64 outstanding_nonblocking_dispatch_stall; 
    W64 marked_branches;
  } dispatch;
  ...
  struct issue {
    ...
    struct width {
      ...
      W64 ready[OutOfOrderModel::IQ_SIZE+1]; // histo: 0, OutOfOrderModel::IQ_SIZE, 1
#endif
    } width;
    W64 wrong_path_instrs;
    W64 blocking_issued;
  } issue;
