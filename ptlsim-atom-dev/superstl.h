...
//ostream logfile;
  ...
  struct bitvecbase {
    ...
    void maskop(size_t count) {
      ...
      w[wordof(count)] &= m;
      // wrap around case for 64
      if(count == BITS_PER_WORD) { w[1] = 0; }
      else
      {
  ...
  public:
    ...
    bitvec<N> remove_print(ostream& logfile, size_t index, size_t count = 1) {
             logfile << "initial: ";
             print(logfile);
             logfile << endl;
             logfile << "stage 0: ";
             ((*this) >> (index + count)).print(logfile);
             logfile << endl;
             logfile << "stage 1: ";
             (((*this) >> (index + count)) << index).print(logfile);
             logfile << endl;
             logfile << "stage 2: ";
              ((*this) % index).print(logfile);
             logfile << endl;
             logfile << "stage 3: ";
             ((((*this) >> (index + count)) << index) | ((*this) % index)).print(logfile);
             logfile << endl; 
