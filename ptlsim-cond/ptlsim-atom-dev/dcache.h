...
#define assert(x) (x)
  ...
  // 32 KB L1 at 4 cycles       // increase to 32 KB to match Core 2
  const int L1_LINE_SIZE = 64;
  const int L1_SET_COUNT = 64;
  const int L1_WAY_COUNT = 8;
  ...
  const int L2_LATENCY   =  12; // don't include the extra wakeup cycle (waiting->ready state transition) in the LFRQ
...
#define ENABLE_L3_CACHE
  ...
  const int L3_LATENCY   =  25; // Core 2 Duo 2.0 GHz has 14 cycle total L2 latency
#endif
