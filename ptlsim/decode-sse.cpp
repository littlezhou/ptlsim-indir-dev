...
#include <decode.h>
static const PermbControlInfo bshift[] = { 
  PermbControlInfo(7,6,5,4,3,2,1,0),
  PermbControlInfo(8,7,6,5,4,3,2,1),
  PermbControlInfo(9,8,7,6,5,4,3,2),
  PermbControlInfo(10,9,8,7,6,5,4,3),   
  PermbControlInfo(11,10,9,8,7,6,5,4),
  PermbControlInfo(12,11,10,9,8,7,6,5),
  PermbControlInfo(13,12,11,10,9,8,7,6),
  PermbControlInfo(14,13,12,11,10,9,8,7),
  PermbControlInfo(15,14,13,12,11,10,9,8)
...
#undef Q
    ...
    {
      case 0x3d3:
      {  uop =  x86_opcode_to_ptl_opcode[0][3];
         sizeshift = x86_opcode_to_sizeshift[0][2];
         break;
      }
#if 0
      case 0x3f3:
      {
         uop =  x86_opcode_to_ptl_opcode[2][3];
         sizeshift = x86_opcode_to_sizeshift[2][3];
         break;

      }
     case 0x3eb:
     {
         uop =  x86_opcode_to_ptl_opcode[1][11];
         sizeshift = x86_opcode_to_sizeshift[1][11];
         break;
    ...
    if(uop == OP_nop) {  logfile << "** Invalid opcode: ",  hexstring(op,64); }
...
#undef Q
  ...
  case 0x5d0: { // addsubpd (SSE3)
    ...
    {  
      ...
      case 0x4d0: {
        this << TransOp(OP_fsub, REG_temp0+0, rdreg+0, rareg+0, REG_zero, 2);
        this << TransOp(OP_fsub, REG_temp1+0, rdreg+1, rareg+1, REG_zero, 2);
        this << TransOp(OP_fadd, REG_temp0+1, rdreg+0, rareg+0, REG_zero, 2);
        this << TransOp(OP_fadd, REG_temp1+1, rdreg+1, rareg+1, REG_zero, 2);
        this << TransOp(OP_permb,rdreg+0, REG_temp0+0, REG_temp0+1, REG_imm, 3, 0,  PermbControlInfo(15,14,13,12,7,6,5,4));
        this << TransOp(OP_permb,rdreg+1, REG_temp1+0, REG_temp1+1, REG_imm, 3, 0,  PermbControlInfo(11,10,9,8,3,2,1,0));
      } 
  ...
  case 0x10f: { // SSSE3 palignr
    ...
    if(imm.imm.imm < 8 && imm.imm.imm >= 0)
    {
       this << TransOp(OP_permb,rdreg+1, rareg+1, rdreg+0, REG_imm, 3, 0,  bshift[imm.imm.imm]);
        this << TransOp(OP_permb,rdreg+0, rareg+0, rareg+1, REG_imm, 3, 0,  bshift[imm.imm.imm]);
 

    }
    else if(imm.imm.imm >= 8 && imm.imm.imm < 16)
    {
        this << TransOp(OP_permb,REG_temp0,rareg+1,rdreg+0, REG_imm, 3, 0,  bshift[imm.imm.imm-8]);
        this << TransOp(OP_permb,rdreg+1,  rdreg+0, rdreg+1, REG_imm, 3, 0,  bshift[imm.imm.imm-8]);
        this << TransOp(OP_mov, rdreg+0, REG_zero,REG_temp0, REG_zero, 3);



    }
    else if(imm.imm.imm >= 16 && imm.imm.imm < 24)
    {


        
        this << TransOp(OP_permb,rdreg+0, rdreg+0, rdreg+1, REG_imm, 3, 0,  bshift[imm.imm.imm-16]);
        this << TransOp(OP_permb,rdreg+1, rdreg+1, REG_zero, REG_imm, 3, 0,  bshift[imm.imm.imm-16]);


    }
    else if(imm.imm.imm >= 24  && imm.imm.imm < 32)
    {
        this << TransOp(OP_permb,rdreg+0, rdreg+1,REG_zero, REG_imm, 3, 0,  bshift[imm.imm.imm-24]);
        this << TransOp(OP_mov, rdreg+1, REG_zero, REG_zero, REG_zero, 3);
  ...
  case 0x216: { // movshdup (SSE3)
    ...
    int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
    int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg]; 
    this << TransOp(OP_permb, rdreg+0,  rareg+0, rareg+0, REG_imm, 3, 0, PermbControlInfo(7,6,5,4,7,6,5,4));
    this << TransOp(OP_permb, rdreg+1,  rareg+1 ,rareg+1, REG_imm, 3, 0, PermbControlInfo(7,6,5,4,7,6,5,4));


    break;
  } 
  case 0x212: { // movsldup (SSE3)
    DECODE(gform, rd, x_mode);
    DECODE(eform, ra, x_mode);
    EndOfDecode();
    ...
    int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
    int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
    this << TransOp(OP_permb, rdreg+0,  rareg+0, rareg+0,REG_imm, 3, 0, PermbControlInfo(3,2,1,0,3,2,1,0));
    this << TransOp(OP_permb, rdreg+1,  rareg+1 ,rareg+1, REG_imm, 3, 0, PermbControlInfo(3,2,1,0,3,2,1,0));
  ...
  default: {
    logfile << "** Invalid SSE opcode: ",  hexstring(op,64); 
    MakeInvalid();
    break;
  }
  }
